const blogEntries = [
  ...
  console.log(getAuthorList(blogEntries))

  const separateClassName = (classNameString) => 
...
const createAuthorElement = (authorObject) =>{
    ...
    authorContainer.classList.add(...separateClassName("author-container d-flex align-items-center gap-3 border border-secondary shadow rounded p-3 mb-3"));
    
    let avatarImage = document.createElement('img')
    avatarImage.classList.add(...separateClassName("author-container__avatar rounded circle"));
...
printUniqueAuthors(getAuthorList(blogEntries), "unique-authors")