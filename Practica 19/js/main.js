...
const createProduct = () => {
    ...
    products.push({...priceproductObject, price: Number(productObject.price)})
    printAllProducts(products, 'product-wrapper');  
    printOptions(getCategories(products), 'category-select')
}

let productSaveButton = document.getElementById('save-product')

productSaveButton.addEventListener("click", (event)=>{
...
const createProductCard = (productObject) => {
    let { name, price, description, picture, category } = productObject;
    ...
    cardText.innerText = description.split(" ").length > 10 ? 
...
filterInput.addEventListener("keyup", (event) => {
    ...
    let result = products.filter((product) => product.category.toLowerCase().includes(query.toLowerCase()))
    console.log(result)
...
const getCategories = (dataArray) => {
    let result = dataArray.reduce((accum, current) => {
        return accum.includes(current.category) ? accum : [...accum, current.category]
}, [])
return result;
}
